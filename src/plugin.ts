...
const plugin: FastifyPluginAsync<InversifyPluginOptions> = async (
  fastify,
  options,
) => {
  ...
  if (disposeOnClose) {
    fastify.addHook(
      "onClose",
      async (instance) => await instance.diContainer.unbindAll(),
    );
  }
  ...
  if (disposeOnResponse) {
    fastify.addHook(
      "onResponse",
      async (request) => await request.diScope.unbindAll(),
    );
  }
};
